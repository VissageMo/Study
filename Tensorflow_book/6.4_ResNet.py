...
def conv2d_same(inputs, num_outputs, kernel_size, stride=1, scope=None):
    if stride == 1:
...
def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections=None, scope=None):
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        preact = slim.batch_nrom(inputs, activation_fn=tf.nn.relu, scope='preact')
        if depth == depth_in:
        ...
        residual = conv2d_same(residual, depth_bottleneck, [1, 1], stride, scope='conv2')
